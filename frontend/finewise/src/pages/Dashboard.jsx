    ...
    <div className="p-6">
      ...
      <section>
        ...
        <div className="space-y-2">
          {transactions.map(tx => (
            <div key={tx._id} className={`p-3 border rounded ${tx.flagged ? 'border-red-400 bg-red-50' : ''}`}>
              <div>{tx.type} — {tx.amount}</div>
              <div className="text-sm text-gray-600">{tx.category} • {new Date(tx.date).toLocaleString()}</div>
              {tx.flagged && <div className="text-xs text-red-600">Flagged: {tx.meta?.fraudReason}</div>}
            </div>
          ))}
        </div>
      </section>
